...
switch ($postType) {
	...
	case ("rowDiscovered"): {
			...
			$stmt = $mysqli->prepare("SELECT COUNT(item_id) FROM items WHERE item_no LIKE '%$search%' OR description LIKE '%$search%' ");
			$stmt->execute();
			$row = $stmt->get_result()->fetch_row();
	...
	case ("view"):
		$rowNum = (isset($_SESSION['itemRowCount']) && $_SESSION['itemRowCount'] != "") ? $_SESSION['itemRowCount'] : 500;
	...
	case ("add"):
		...
		$stmtCheck->fetch();
		if ($found) {
			$errorMessage = "Item Already Existed";
			echo $errorMessage;
