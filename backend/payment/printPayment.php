...
switch ($postType) {
    ...
    case ("paymentList"):
		...
		$stmt = $mysqli->prepare("SELECT customer_id, payment_identifier, invoice_id, payment_date, payment_mode, payment_remark, payment_salesperson, customer_account FROM payment ORDER BY payment_id desc limit $recordsPerPage OFFSET $offsetValue"); 
    ...
    case ("printPayment"):
        if (isset($_POST["payment_identifier"], $_POST["customer_account"] )) {
            ...
            if (mysqli_num_rows($result) > 0) {
				...
				$jsonArrayPayment[] = $_POST["payment_identifier"];
