...
switch ($postType) {
	...
	case ("viewInvoiceAll"):
		if (isset($_POST["account_num"])) {
			...
			$stmt = $mysqli->prepare("SELECT id, invoice_id, doc_no, creation_date, invoice_num,  invoice_date, due_date, total_amount, outstanding FROM invoice_header WHERE in_account = ? ORDER BY id limit $recordsPerPage OFFSET $offsetValue"); 
			$stmt->bind_param("s", $_POST["account_num"]);
			$stmt->execute();
			$result = $stmt->get_result();
			// Check number of rows in the result set
